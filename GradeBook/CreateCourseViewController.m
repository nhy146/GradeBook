...
@synthesize cnameTextField,descriptionTextView, ekeyTextField, startTextField, endTextField, schoolTextField, locationTextField, startDatePicker, endDatePicker, accessoryView, teacher; //activeField;
...
- (IBAction)createcourseButton:(id)sender {
    ...
    NSURL *url=[NSURL URLWithString:@"http://lsucptg434gb.summerhost.info/app_file/tcreate_new_course.php"];
    // cname, desc, ekey, start, end, school, tid
    NSString *post =[NSString stringWithFormat:@"cname=%@&desc=%@&ekey=%@&start=%@&end=%@&school=%@&tid=%@&location=%@",[cnameTextField text],[descriptionTextView text],[ekeyTextField text],[startTextField text],[endTextField text],[schoolTextField text],teacher.tid,[locationTextField text]];
